...
-- 형식 INSERT INTO 테이블명(열_이름1,2,3,...) VALUES (값1,2,3,...);
INSERT INTO dept_temp (DEPTNO, DNAME, LOC)
VALUES (50, 'DATABASE', 'SEOUL');
DELETE FROM DEPT_TEMP WHERE DEPTNO = 50;

-- 칼럼수와 값의 수가 같으면 칼럼 나열 생략가능
INSERT INTO dept_temp
VALUES (60, 'DATABASE', 'SEOUL');

-- SQL Error [947] [42000]: ORA-00947: 값의 수가 충분하지 않습니다 -- 칼럼수와 값의 수가 다르면 안됨
INSERT INTO dept_temp (DEPTNO, DNAME, LOC)
VALUES (70, 'DATABASE');
...
-- 날짜 년/월/일 => 일/월/년 순으로 입력은 안됨 : 기본형식이 년/월/일
INSERT INTO EMP_TEMP(EMPNO,ENAME,JOB,HIREDATE,SAL,DEPTNO) VALUES (8888, '성춘향', 'SALESMAN', TO_DATE('07/08/2023', 'DD/MM/YY'),3000,20);
...
SELECT E.EMPNO, E.ENAME, E.JOB, E.HIREDATE,E.SAL, E.COMM,E.DEPTNO FROM EMP e JOIN SALGRADE s ON E.SAL BETWEEN S.LOSAL AND S.HISAL 
WHERE S.GRADE = 1;

-- UPDATE(수정)
SELECT * FROM DEPT_TEMP;

-- 형식 : UPDATE 테이블명 SET 변경할 칼럼 = 값, ... WHERE 변경할 대상 행 조건;
...
-- DEPT 테이블의 DEPTNO = 40 부서의 DNAME, LOC를 추출해서 DEPT_EMPT2의 DNAME, LOC수정
UPDATE DEPT_TEMP2
SET (DNAME, LOC) = 
...
-- 형식 : DELETE 테이블명 WHERE 삭제할 조건
--		 DELETE FROM 테이블명 WHERE 삭제할 조건 -- FROM 생략가능
--		 DELETE 테이블명
SELECT * FROM EMP_TEMP;
...
-- EMP테이블 행 추출 후 EMP_TEMP2에 삽입 
INSERT INTO EMP_TEMP2 (EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO)
SELECT * FROM EMP;

-- 서브쿼리
-- 급여의 등급이 3등급이고, 부서번호가 30번인 사원만 삭제

DELETE 
FROM 
EMP_TEMP2 IN
WHERE EMPTNO IN
(SELECT EMTNO FROM EMP_TEMP2 E JOIN SALGRADE s ON  BETWEEN S.LOSAL AND S.HISAL AND S.GRADE = 3 AND E.DEPTNO = 30);  
...
-- INSERT
-- EXAM_DEPT 테이블에 50,60,70,80부서 등록
...
-- 7207, TEST_USER7, LECTURE, 7201, 2016-10-28,4500,NULL,80
...
INSERT INTO EXAM_EMP VALUES(7207, 'TEST_USER7', 'LECTURE', 7201, '2016-10-28',4500,NULL,80);
...
-- EXAM_EMP에 속한 사원 60번 부서의 사원 중에서 입사일이 가장 빠른 사원보다 늦게 입사한 사원의 급여를 10%인상하고 80번 부서로 변경
UPDATE EXAM_EMP
SET DEPTNO = 80, SAL = 1.1*SAL
WHERE HIREDATE < (SELECT MAX(HIREDATE) FROM EXAM_EMP WHERE DEPTNO = 60);


-- DELETE 
-- EXAMP_EMP에 속한 사원 중 급여 등급이 5인 사원 삭제
DELETE
	EXAM_EMP
WHERE
	EMPNO IN
...
-- TCL은 자동커밋이 안됨 => 데이터변화
-- TRANSACTION : 한번에 수행되야하는 논리적 단위
-- EX) 은행계좌 이체 : 내 통장에서 인출하고 다른계좌로 입금이라는 작업을 한 단위로 트랜잭션이라 함
-- COMMIT(반영), ROLLBACK(취소) : ORACLE에서는 DML은 자동커밋 X 명시해야 트랜잭션 가능
-- SQL SERVER에서는 모두 자동커밋
-- DBEAVER 편집기는 자동커밋 제공
