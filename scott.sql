...
-- 급여 높은순으로 TOP-N 추출 / 페이지 나눌때 사용
SELECT ROWNUM, e.*
FROM (SELECT * FROM EMP ORDER BY SAL DESC) e
WHERE ROWNUM <= 3;

-- 시퀀스 : 규칙에 따라 순번을 생성 / 은행의 번호표 개념
-- 기본형식 : CREATE SEQUENCE 시퀀스_이름;
CREATE SEQUENCE DEPT_SEQ;

-- 내부적으로는
-- INCREMENT BY 1 (기본값 : 1, 입력값만큼 증가)
...
MAXVALUE 99
CYCLE;
-- 수정한 부분만 변경 / 사이클 돌아갈때는 최솟값부터 시작 / NEXTVAL를 나중에 기본키로 사용할 거라 사이클은 잘안씀
INSERT INTO DEPT_SEQUENCE(DEPTNO, DNAME,LOC)
VALUES (DEPT_SEQ.NEXTVAL,'DATABASE', 'SEOUL');

-- 동의어 : 테이블이나 뷰, 시퀀스와 같은 객체에 대신 사용할 수 있는 이름 부여 ( 변수개념 )
-- CREATE SYNONYM 동의어_이름 FOR 테이블_명;
...
-- 제약 조건 : 특정 칼럼에 지정
-- 1) NOT NULL(NN) : 지정한 열에 NULL허용X
-- 2) UNIQUE : 지정한 열이 유일한 값을 가져야 함
-- 3) PRIMARY KEY : 지정한 열이 유일한 값이면서 NULL 허용하지 않음 ( 테이블 당 하나만 지정가능 )
-- 4) FOREIGN KEY : 다른 테이블의 열을 참조하여 존재하는 값만 입력, NULL허용
...
-- NULL을 ("SCOTT"."TABLE_NN"."LOGIN_PWD") 안에 삽입할 수 없습니다
INSERT INTO TABLE_NN VALUES(
	'TESE_ID_01', NULL, '010-1234-5678'
);

INSERT INTO TABLE_NN VALUES(
	'TESE_ID_01', 'TESE_PWD_01', NULL
);
...
-- 제약조건에 이름 설정 : CONSTRAINT + 조건_이름
CREATE TABLE TABLE_NN2(
	LOGIN_ID VARCHAR2(20) CONSTRAINT TBLNM2_LGID_NN NOT NULL,
	LOGIN_PWD VARCHAR2(20) CONSTRAINT TBLNM2_LGPWD_NN NOT NULL,
	TEL VARCHAR2(20)
);

-- 기존 테이블에 제약 조건 추가(TEL에 제약조건 추가)
-- 사용으로 설정 불가 - 널 값이 발견되었습니다. 기존에 NULL값이 있으므로 오류
ALTER TABLE TABLE_NN MODIFY ( TEL NOT NULL);

UPDATE TABLE_NN SET TEL = '010-5678-9358' WHERE LOGIN_ID = 'TEST_ID_10';
ALTER TABLE TABLE_NN MODIFY (TEL CONSTRAINT TBLNM_TEL_NN NOT NULL);

-- 생성한 제약 조건 이름 변경
ALTER TABLE TABLE_NN RENAME CONSTRAINT TBLNM_TEL_NN TO TBLNM2_TEL_NN;

-- 제약 조건 삭제
ALTER TABLE TABLE_NN2 DROP CONSTRAINT TMLNN2 _TEL_NN;

-- 변수명 지정 ( NAMING RULE )
-- JAVA : 소문자 작성, 두 단어 이상의 조합은 2번째 단어부터 첫글자만 대문자 작성(카멜표기법 == 낙타법)
-- DB : 두 단어 이상의 조합은 단어와 단어 사이를 _로 구분 (스네이크 표기법)

-- 2) UNIQUE : 데이터 중복을 허용하지 않음, NULL 허용
...
-- 3) PK(기본키) : NOT NULL + UNIQUE, 하나의 칼럼만 지정가능
-- EX) 주민번호, 사원번호, ID
...
INSERT INTO TABLE_PK VALUES (NULL, 'TEST_PWD_01', '010-1234-5678');

INSERT INTO TABLE_PK VALUES ('TEST_ID_01', 'TEST_PWD_01', '010-1234-5678');
-- 한 번 더 입력시 무결성 제약 조건(SCOTT.SYS_C008371)에 위배됩니다
INSERT INTO TABLE_PK VALUES ('TEST_ID_01', 'TEST_PWD_01', '010-1234-5678');
...
--	UNIQUE(LOGIN_PWD)
	CONSTRAINT TBL_UNK UNIQUE(LOGIN_PWD) -- 제약 조건을 아래에 선언도 가능
);

-- 4) FOREIGN KEY (FK - 외래키) : 다른 엔터티의 기본키를 참조하는 칼럼

-- 무결성 제약조건(SCOTT.FK_DEPTNO)이 위배되었습니다- 부모 키가 없습니다
-- DEPT테이블의 DEPTNO를 참조하기 때문에 DEPTNO의 값을 EMP테이블에 값을 저장해야 함
INSERT INTO EMP e 
...
-- BOARD 테이블 정의
-- BNO(숫자), NAME, PASSWORD, TITLE, CONTENT, READCNT(숫자, DEFAULT 0), REGDATE(날짜 - DEFAULT 현재 날짜) - 나머지 문자
-- BNO : PK
-- NAME, PASSWOR, TITLE, CONTENT : NOT NULL

-- 시퀀스 생성 : 1씩 증가 BOARD_SEQ

CREATE TABLE BOARD(
	BON NUMBER(8) PRIMARY KEY,
...
INSERT INTO BOARD (BON,NAME,PASSWORD,TITLE,CONTENT)
VALUES (BOARD_SEQ.NEXTVAL, 'HONH', '1111','게시판', '게시글 작성');
