...
FROM EMPLOYEES e JOIN EMPLOYEES e2 ON E.MANAGER_ID = E2.EMPLOYEE_ID AND E.HIRE_DATE < E2.HIRE_DATE ;

-- 도시 이름이 T로 시작하는 지역에 사는 사원들의 정보 조회
-- 사원번호, 이름(LAST_NAME), 부서번호, 지역명
-- EMPLOYEES, DAPARTMENT, LOCATIONS 조인
SELECT
	E.EMPLOYEE_ID ,
	E.LAST_NAME ,
	D.DEPARTMENT_ID ,
...
JOIN JOB_HISTORY jh ON
	E.EMPLOYEE_ID = JH.EMPLOYEE_ID AND E.JOB_ID = JH.JOB_ID;


-- 각 사원별 소속부서에서 자신보다 늦게 고용되었으나 많은 급여를 받는 사원의 정보조회
-- 부서번호, FIRST_NAME 과 LAST_NAME을 연결하여 출력(CONCAT), 급여, 입사일 출력
-- EMPLOYEES SELF 조인
SELECT
